...
module TwoWays {
  ...
  var dataBindingListInternal: TestObject[] = []; 
  var bindingL: WinJS.Binding.List<TestObject>;
  var dataBindingList: WinJS.Binding.GroupedSortedListProjection<TestObject>;
  ...
  WinJS.UI.processAll().done(function () {
    ...
    bindingL = new WinJS.Binding.List<TestObject>(dataBindingListInternal);
  ...
  function GroupKey(item): any {
    var group = groups.filter(function (element, index, array): boolean {
      return (element.id == item.group.id);
    })[0];
    if (group == undefined) {
      groups.push({ id: item.group.id, title: 'Group ' + item.group.id });
      group = groups.filter(function (element, index, array): boolean {
        return (element.id == item.group.id);
      })[0];
    }
    return group.title;
  ...
  function GroupSorter(leftKey, rightKey): number {
    var r = 0;
    if (leftKey < rightKey) { r = -1; }
    else if (leftKey == rightKey) { r = 0; }
    else { r = 1; }
    return r;
  };
  ...
  function PopulateListView() {
    ...
    if (listView === null || listView.winControl === undefined) return;
...
class TestObject {
  ...
  constructor(t, i, b, g) { this.title = t; this.image = i; this.group = g; this.backColor = b; }
...
class TestGroup {
  ...
  constructor(i, sg) { this.id = i; this.subGroup = sg; }
}
